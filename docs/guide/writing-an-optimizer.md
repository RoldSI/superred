...
it is your side of the contract. If you do not return -- because you are blocked
on something the channel close cannot reach -- you are cancelled once the
cleanup budget is spent, and so is a `teardown()` that does not finish inside
it. Do any work that must complete before the channel closes, not after.
