...
`_print_summary`. It now narrates the whole run through a **reporter**: an
observer object (`superred.core.reporting.ProgressReporter`) that the Controller
calls at each lifecycle point (threat-model start, task start, each run, task
...
It is now on by default and writes to a results root.
...
whole results root as sensitive. It is gitignored by default.
