...
holds a measurement are kept and not re-run; only `error`, interrupted, or
missing tasks recompute. (The kept set has since gained a fourth member,
`timeout` -- a task the wall-clock cap truncated after at least one run had been
judged -- and the rule is now exposed as `persistence.is_kept`. See
[Results & Persistence](/reference/results#resume).) A task is
