...
- `evaluate(trajectory, target) -> EvaluationResult` — query post-run ground truth via `target.query()`, assess success. Each `Score` carries a `security_domain` (`SecurityDomainTag | None`); the controller filters `sub_scores` by scope. `None` means the score is always visible.
