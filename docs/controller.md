...
1. **Per task** (from security claim):
   ...
   - **Run loop** (until optimizer signals done or `max_runs_per_task`):
     ...
     - `task.evaluate(trajectory, target)` — returns `EvaluationResult`. Controller filters `sub_scores` by scope (keeping only in-scope scores), emits a `FeedbackEvent(evaluation=filtered_eval, security_domain=scope_tag)` to the trajectory, then closes it.
...
- `_run_single(task, channel, scope, run_number)` — executes one iteration: RunStartEvent → target.run → RunEndEvent → evaluate → feedback → close trajectory. Returns `(trajectory, evaluation, done)`.
